...
struct list* list_create(size_t size, size_t data_size)
{
    ...
    struct list list = {size, 0, data_size, data};
    list_ptr = malloc(sizeof(struct list) + size * data_size);
    *list_ptr = list;
    return list_ptr;
}
...
void list_append(struct list* list_ptr, void* data)
{
    if (list_ptr->count >= list_ptr->size)
    {
        ...
        {
            data_buf[i * list_ptr->data_size] = &*list_ptr->data[i * list_ptr->data_size];
        }
        if (!data_buf)
        {
    ...
    list_ptr->data[list_ptr->count * list_ptr->data_size] = data;
    list_ptr->count++;
}
...
{
    return list_ptr->data + index * list_ptr->data_size;
}
